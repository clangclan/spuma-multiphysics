...
int main(int argc,char** argv){
    ...
    f.read((char*)header,sizeof(header));assert(header[0]==0x48454d4341503031ULL&&header[2]==sizeof(ReactiveDeviceFlash::Model));
    ReactiveDeviceFlash::Model model{};f.read((char*)&model,sizeof(model));assert(f.good());
    ...
    for(int phase=0;phase<=model.nl;++phase){const auto& table=model.phase[phase];
        for(int k=0;k<table.ns;++k)for(int r=0;r<table.species[k].regionCount;++r){
            ...
            for(int i=0;i<=2000;++i){const double T=low+(high-low)*i/2000.;double ref[3]{},single[3]{},ds[3]{};
                ReactiveDevicePR::detail::nasa(region,T,ref[0],ref[1],ref[2]);
                assert(ReactiveDevicePR::detail::mixedNasa<float>(region,T,single[0],single[1],single[2]));
                assert(ReactiveDevicePR::detail::mixedNasa<ReactiveDevicePR::detail::DoubleSingle>(region,T,ds[0],ds[1],ds[2]));
