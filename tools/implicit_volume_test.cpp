...
int main(int argc,char**argv){
 ...
 ReactiveImplicitSurface::View v{coefficients.data(),{n,n,n},{0,0,0},{1,1,1}};
 double volume=0,area=0;int fails=0;unsigned eval=0;
 for(int k=0;k<n;++k)for(int j=0;j<n;++j)for(int i=0;i<n;++i){
  int64_t cell[3]={i,j,k};ReactiveImplicitVolume::Integrals r{};
  const bool ok=ReactiveImplicitVolume::integrate(v,cell,{1e-6,8},r);
