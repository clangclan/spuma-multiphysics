...
for mode in "${modes[@]}"; do
    flags=()
    case "$mode" in
        fp32-seeds) flags+=(-DPINTLE_HEM_FP32_SEEDS=1);;
        fp32-linear) flags+=(-DPINTLE_HEM_FP32_LINEAR=1);;
        int32-linear) flags+=(-DPINTLE_HEM_INT_LINEAR=1);;
        fp32-combined) flags+=(-DPINTLE_HEM_FP32_SEEDS=1 -DPINTLE_HEM_FP32_LINEAR=1);;
        fp32-nasa) flags+=(-DPINTLE_HEM_NASA_PRECISION=1);;
        ds-nasa) flags+=(-DPINTLE_HEM_NASA_PRECISION=2);;
        fp64);;
    ...
    "${NVCC:-nvcc}" -std=c++17 -O2 -Xcompiler=-fPIC --fmad=false \
        ...
        "${flags[@]}" -shared "$root/src/reactiveTransport/pintleGpuHemFma.cu" \
