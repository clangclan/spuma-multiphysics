...
def main():
    ...
    with RealFluidBackend(a.config,a.library) as b:
        ...
        def pool(budget=8*1024*1024):
            error=C.create_string_buffer(1024);handle=b.lib.pintle_rt_pool_create(b.handle,4,n,budget,error,len(error));return handle,error.value.decode()
        latest={}
        def batch(handle,token=Token(1,1,1),negative=False):
            ...
            rc=b.lib.pintle_rt_pool_batch(handle,token,0,n,b.ns,ptr(mass),ptr(energy),states,0,1e-8,1e-14,C.byref(drift))
            latest['state']=states[0].copy()
            return rc,before==(mass.tobytes(),energy.tobytes(),bytes(states)),b.lib.pintle_rt_pool_error(handle).decode()
        original=b.policy_hash;rc=b.lib.pintle_rt_set_closure_acceleration_v1(b.handle,9,0,b'')
        record('invalid-mode-does-not-change-policy',rc!=0 and original==b.policy_hash)
        rc=b.lib.pintle_rt_set_closure_acceleration_v1(b.handle,1,1,b'/missing/closure-library.so')
        ...
        if handle:b.lib.pintle_rt_pool_destroy(handle)
        ...
        try:
            ...
            os.environ['PINTLE_TEST_BAD_CLOSURE_CANDIDATE']='1'
            try:rc,_,error=batch(handle,Token(2,1,2));stats=profile(b,handle)
            finally:os.environ.pop('PINTLE_TEST_BAD_CLOSURE_CANDIDATE',None)
        ...
        finally:b.lib.pintle_rt_pool_destroy(handle)
        ...
        try:
            ...
            rc=b.lib.pintle_rt_pool_acceleration_profile_v1(handle,C.byref(wrong))
        ...
        try:
            ...
            fn=b.lib.pintle_rt_pool_failure_v1;fn.argtypes=[C.c_void_p,C.c_size_t,C.POINTER(Failure),C.POINTER(C.c_char_p)];fn.restype=C.c_int
