...
int main(int argc,char** argv) {
    ...
    const double h=ReactiveGeometricOracle::boxLength/n,r=.00074;
    ...
    ReactiveImplicitSurface::View field{coefficient.data(),{n,n,n},{-.002,-.002,-.002},{h,h,h}};
    double aperture=0,traction=0,volumeIdentity=0,surfaceIdentity=0;int failures=0;
    for(int k=0;k<n;++k)for(int j=0;j<n;++j)for(int i=0;i<n;++i) {
        const int64_t cell[3]={i,j,k};
        ReactiveImplicitVolume::Integrals v{};
        if(!ReactiveImplicitVolume::integrate(field,cell,{1e-8,12},v)){++failures;continue;}
        double ar[3]{},cr[3]{};
        for(int axis=0;axis<3;++axis)for(int side=0;side<2;++side) {
            ReactiveImplicitFace::Integrals f{};
            if(!ReactiveImplicitFace::integrate(field,cell,axis,side,{1e-8,12},f)){++failures;continue;}
            std::array<int,3> index{i,j,k};index[axis]-=1-side;
            const auto exact=ReactiveGeometricOracle::exactFace(n,axis,index,center,r,1);
