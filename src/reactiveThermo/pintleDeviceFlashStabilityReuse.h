...
#ifndef PINTLE_DEVICE_FLASH_STABILITY_REUSE_H
#define PINTLE_DEVICE_FLASH_STABILITY_REUSE_H

#include "pintleDeviceFlash.h"

#include <cstdint>

namespace PintleDeviceFlashStabilityReuse {
...
struct RecoveryFlash : PintleDeviceFlash::Flash {
    Stats& stats;

    PINTLE_HD RecoveryFlash(const Model& model,const Input& input,
                            Counters& counters,Stats& reuseStats)
        : PintleDeviceFlash::Flash(model,input,counters),stats(reuseStats) {}
    ...
    PINTLE_HEM_CALL bool stableFromActiveFlash(const Evaluation& value) {
        ...
        for(int i=0;i<m.nl;++i){
            ...
            if(value.state.liquidMass[i]>0){
                if(!PintleDeviceFlash::finite(affinity)
    ...
    PINTLE_HD std::uint64_t repeatedVirtualPhaseRequests() const {
    ...
    PINTLE_HD void consider(Evaluation& candidate,Evaluation& best,bool& have,
        ...
        }else accepted=PintleDeviceFlash::Flash::stable(candidate);
        if(accepted){
            ++count.stable;
    ...
    PINTLE_HEM_CALL bool search(Evaluation& best) {
        if(hasSolid())return PintleDeviceFlash::Flash::search(best);
        ...
        if(!have)have=PintleDeviceFlash::Flash::temperatureSearch(best);
        return have;
    }

    // This is the canonical boundary-recovery policy and winner comparison.
    PINTLE_HEM_CALL bool run(Evaluation& result) {
        if(hasSolid())return PintleDeviceFlash::Flash::run(result);
        if(m.ns<=0||m.ns>PintleDeviceFlash::maxSpecies||m.nl<0||m.nl>2
           ||!PintleDeviceFlash::finite(in.energy)||!(density()>0))return false;
        for(int k=0;k<m.ns;++k)
            if(!PintleDeviceFlash::finite(in.q[k])||in.q[k]<0)return false;
...
PINTLE_HD inline Result recover(const Model& model,const Input& input) {
