...
#ifndef PINTLE_REAL_FLUID_H
#define PINTLE_REAL_FLUID_H
#include "pintleReactiveThermo.h"
#include <stdint.h>
#ifdef __cplusplus
extern "C" {
#endif
enum { PINTLE_RF_BASIC=1, PINTLE_RF_REFERENCE=2, PINTLE_RF_DERIVATIVES=4,
       PINTLE_RF_PARTIAL_H=8, PINTLE_RF_CHEMICAL_POTENTIAL=16 };
typedef struct PintleRealFluidCapabilities {
...
} PintleRealFluidCapabilities;
typedef struct PintleRealFluidResult {
...
} PintleRealFluidResult;
typedef struct PintleRealFluidProfile {
...
} PintleRealFluidProfile;
typedef struct PintleThermoTangentResult {
    uint32_t abiVersion,structBytes;
...
} PintleThermoTangentResult;
const char* pintle_rt_physical_model_hash(void* model);
const char* pintle_rt_numerical_policy_hash(void* model);
const char* pintle_rt_eos_name(void* model);
...
int pintle_rt_set_case_context(void* model,const char* physical,const char* numerical);
int pintle_rt_capabilities(void* model, PintleRealFluidCapabilities* result);
int pintle_rt_real_fluid_profile(void* model, int reset, PintleRealFluidProfile* result);
// Reject unknown keys and unsupported policy values. No EOS switching.
int pintle_rt_load_optimization_policy(void* model,const char* filename);
...
int pintle_rt_evaluate_real_fluid(void* model,double T,double rho,const double* Y,
    int phase,uint32_t mask,const size_t* selected,size_t selectedCount,
    PintleRealFluidResult* result,double* partialMassH,double* chemicalPotential);
...
int pintle_rt_thermo_tangent(void* model,const double* q,double energy,
    const PintleThermoState* state,const double* direction,double energyDirection,
    PintleThermoTangentResult* result);
...
int pintle_rt_chemical_matrix_free_jvp(void* model,const double* q,double energy,
    int equilibrium,const PintleThermoState* state,const double* direction,
...
const char* pintle_rt_pool_error(void* pool);
typedef struct PintleBatchToken { uint64_t attemptId,stageId,contentVersion; } PintleBatchToken;
typedef struct PintleBatchProfile {
