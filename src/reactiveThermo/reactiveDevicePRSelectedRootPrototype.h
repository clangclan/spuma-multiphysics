...
#ifndef REACTIVE_DEVICE_PR_SELECTED_ROOT_PROTOTYPE_H
#define REACTIVE_DEVICE_PR_SELECTED_ROOT_PROTOTYPE_H

#include "reactiveDevicePR.h"

#include <cstdint>

namespace ReactiveDevicePRSelectedRootPrototype {
...
REACTIVE_HD inline void classifySingle(double T,double a,double b,double center,
                                     double volume,SelectedRoot& result) {
    const double tc=a*ReactiveDevicePR::OmegaB
        /(b*ReactiveDevicePR::OmegaA*ReactiveDevicePR::GasConstant);
    const double pc=ReactiveDevicePR::OmegaB*ReactiveDevicePR::GasConstant*tc/b;
    const double vc=ReactiveDevicePR::OmegaVc*ReactiveDevicePR::GasConstant*tc/pc;
...
REACTIVE_HD inline Status fromCanonical(double T,double p,double a,double b,
                                      double aa,RootChoice choice,
                                      SelectedRoot& output,Stats* stats,
                                      bool requireAvailability) {
    RootSet roots{};
    const Status status=ReactiveDevicePR::detail::rootsFromMixingCached(
        T,p,a,b,aa,roots);
...
REACTIVE_HD inline Status rootsFromMixingSelected(
    ...
    if(!ReactiveDevicePR::detail::finite(T)||!ReactiveDevicePR::detail::finite(p)
       ||!ReactiveDevicePR::detail::finite(a)||!ReactiveDevicePR::detail::finite(b)
       ||!ReactiveDevicePR::detail::finite(aa)||T<=0||p<=0||a<=0||b<=0||aa<=0)
    ...
    double disc=q*q-h*h;
    if(ReactiveDevicePR::detail::abs(ReactiveDevicePR::detail::abs(h)
                                  -ReactiveDevicePR::detail::abs(q))<1e-10){
        if(disc>1e-10)return Status::NonFinite;
        disc=0;
    }
    if(ReactiveDevicePR::detail::abs(disc)<1e-14)
        return Status::NoPhysicalRoot;

    double selectedVolume=0;
    int physicalCount=0;
    if(disc>1e-14||!(delta2>0)){
        const double sd=.5*::sqrt(ReactiveDevicePR::detail::max(0.0,disc));
        selectedVolume=center+ReactiveDevicePR::detail::cubeRoot(-.5*q+sd)
            +ReactiveDevicePR::detail::cubeRoot(-.5*q-sd);
        if(stats)++stats->oneRealRoot;
        for(int n=0;n<12;++n){
            const double residual=ReactiveDevicePR::detail::cubicResidual(
                selectedVolume,bn,cn,dn);
            const double deriv=(3.0*selectedVolume+2.0*bn)*selectedVolume+cn;
            if(ReactiveDevicePR::detail::abs(deriv)<=1e-300)break;
            const double step=residual/deriv;selectedVolume-=step;
            if(ReactiveDevicePR::detail::abs(step)<=4e-15
               *ReactiveDevicePR::detail::max(
                    1.0,ReactiveDevicePR::detail::abs(selectedVolume)))break;
        }
        if(!ReactiveDevicePR::detail::finite(selectedVolume)||selectedVolume<=b)
            return Status::NoPhysicalRoot;
        physicalCount=1;
    }else if(disc<-1e-14){
        if(stats)++stats->threeRealRoots;
        double arg=-.5*q/(delta*delta*delta);
        arg=ReactiveDevicePR::detail::max(-1.0,
              ReactiveDevicePR::detail::min(1.0,arg));
        ...
        if(!ReactiveDevicePR::detail::finite(selectedVolume))
        ...
        if(!(qdisc>=0)||!ReactiveDevicePR::detail::finite(qdisc))
            return detail::fromCanonical(T,p,a,b,aa,choice,output,stats,
                                         requireAvailability);
        const double sd=::sqrt(qdisc);
        const double other0=.5*(-qb-sd),other1=.5*(-qb+sd);
        const double magnitude=ReactiveDevicePR::detail::max(
            1.0,ReactiveDevicePR::detail::max(
                ReactiveDevicePR::detail::abs(selectedVolume),
                ReactiveDevicePR::detail::max(
                    ReactiveDevicePR::detail::abs(other0),
                    ReactiveDevicePR::detail::abs(other1))));
        const double guard=1e-10*magnitude;
        if(!ReactiveDevicePR::detail::finite(other0)
           ||!ReactiveDevicePR::detail::finite(other1)
           ||ReactiveDevicePR::detail::abs(selectedVolume-b)<=guard
           ||ReactiveDevicePR::detail::abs(other0-b)<=guard
           ||ReactiveDevicePR::detail::abs(other1-b)<=guard)
            return detail::fromCanonical(T,p,a,b,aa,choice,output,stats,
                                         requireAvailability);
        if(liquid){
            if(selectedVolume>b
...
REACTIVE_HD inline Status reportedPhaseSelected(
    ...
    Status status=ReactiveDevicePR::detail::mixingRootsFixed<NS>(
    ...
    if(!found||!(liquidVolume>0)
       ||!ReactiveDevicePR::detail::finite(liquidVolume))
...
template<int NS,int NC,int NR>
REACTIVE_HD inline Status evaluateTrhoFromMixingSelected(
    ...
    if(NS<1||NC<NS||table.ns!=NS||!x
       ||!ReactiveDevicePR::detail::finite(T)
       ||!ReactiveDevicePR::detail::finite(rho)||T<=0||rho<=0
       ||!ReactiveDevicePR::detail::finite(W)
       ||!ReactiveDevicePR::detail::finite(a)
       ||!ReactiveDevicePR::detail::finite(b)
       ||!ReactiveDevicePR::detail::finite(aa)
       ||!ReactiveDevicePR::detail::finite(da)
       ||!ReactiveDevicePR::detail::finite(d2a)
    ...
    if(!ReactiveDevicePR::detail::finite(s.p)||s.p<=0)
        return Status::NoPhysicalRoot;
    s.dpdV=-GasConstant*T/(vmb*vmb)+2.0*aa*(s.V+b)/(den*den);
    s.dpdT=GasConstant/vmb-da/den;
    if(!ReactiveDevicePR::detail::finite(s.dpdV)
       ||!ReactiveDevicePR::detail::finite(s.dpdT)||s.dpdV>=0)
    ...
    for(int k=0;k<table.ns;++k){
        ...
        ReactiveDevicePR::detail::nasa(
            ReactiveDevicePR::detail::region(table,k,T),T,cp_R,h_RT,s_R);
    ...
    for(double value:values)
        if(!ReactiveDevicePR::detail::finite(value))return Status::NonFinite;
    if(chemicalPotentials)for(int k=0;k<table.ns;++k)
        if(!ReactiveDevicePR::detail::finite(s.mu[k]))return Status::NonFinite;
    output=s;return Status::Success;
}

template<int NS,int NC,int NR>
REACTIVE_HD inline Status evaluateTPTrustedFixed(
    ...
    double x[NS]{},W=0;
    Status status=ReactiveDevicePR::massToMole(table,Y,x,W);
    if(status!=Status::Success)return status;
    double a=0,b=0,aa=0,da=0,d2a=0;
    status=ReactiveDevicePR::detail::mixingFixed<NS>(
    ...
    status=evaluateTrhoFromMixingSelected<NS>(
        ...
        stats);
    if(status!=Status::Success)return status;
    if(ReactiveDevicePR::detail::abs(state.p-p)>1e-7
       *ReactiveDevicePR::detail::max(1.0,p))return Status::NonFinite;
