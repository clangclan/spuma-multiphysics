...
// Handle for reactive_gpu_hem_run_resident_v1 only: model, phase cache and
// per-cell counters; host-path staging is allocated only if a host batch runs.
void* reactive_gpu_hem_create_resident_v1(const void* model,size_t modelBytes,size_t capacity,char* error,size_t errorSize);
// Device bytes currently held by a handle.
size_t reactive_gpu_hem_device_bytes_v1(void*);
...
int reactive_gpu_hem_initialize_tp_v1(void*,double* q,double* energy,
    const ReactiveGpuHemCapillaryInputV2* capillary,size_t count,
    ReactiveThermoState* state,int* success,ReactiveGpuHemProfileV1* profile,char* error,size_t errorSize);
// Device-resident capillary batch. Every pointer is DEVICE memory indexed by
// global cell: q (ns masses per cell), bulk energy, seed state and capillary
// input. `order[0..count)` names the cells to flash; each result goes to
// output[cell]. Nothing is copied to or from the host except batch totals.
// The HEM must have been created with capacity >= count.
typedef struct ReactiveGpuHemOutputV1 {
    ReactiveThermoState state;int success,reserved;
} ReactiveGpuHemOutputV1;
int reactive_gpu_hem_run_resident_v1(void*,const double* q,const double* energy,
    ...
    ReactiveGpuHemProfileV1* profile,char* error,size_t errorSize);
