...
#ifndef REACTIVE_SOLID_THERMO_H
#define REACTIVE_SOLID_THERMO_H

#include <cmath>

#ifdef __CUDACC__
#define REACTIVE_SOLID_HD __host__ __device__
#else
#define REACTIVE_SOLID_HD
#endif

namespace ReactiveSolidThermo {
...
REACTIVE_SOLID_HD inline bool finite(double x) {
#ifdef __CUDA_ARCH__
    return isfinite(x);
#else
...
REACTIVE_SOLID_HD inline bool validModel(const Model& m) {
...
REACTIVE_SOLID_HD inline int segment(const Model& m,double T) {
    if(T<=m.T[0])return 0;
    for(int i=0;i<m.n-1;++i)if(T<m.T[i+1])return i;
    return m.n-2;
}

REACTIVE_SOLID_HD inline double heatCapacity(const Model& m,double T) {
...
REACTIVE_SOLID_HD inline void integrateAscending(const Model& m,double lower,double upper,
...
REACTIVE_SOLID_HD inline bool evaluate(const Model& m,double T,double p,State& out) {
