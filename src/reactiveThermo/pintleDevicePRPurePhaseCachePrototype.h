...
// Prototype invariant reported-phase cache for one-species PR tables.
#ifndef PINTLE_DEVICE_PR_PURE_PHASE_CACHE_PROTOTYPE_H
#define PINTLE_DEVICE_PR_PURE_PHASE_CACHE_PROTOTYPE_H

#include "pintleDevicePR.h"

#include <cfloat>

namespace PintleDevicePRPurePhaseCachePrototype {
...
struct PureReportedPhaseCache {
    ...
    double densitySlope=0;
    int valid=0;
};

template<int NC,int NR>
PINTLE_HD inline Status buildPureReportedPhaseCache(
    const PintleDevicePR::Table<NC,NR>& table,
    ...
    result.densitySlope=(criticalDensity-densityMidAtTmid)/(tc-tmid);
    result.valid=detail::finite(result.criticalTemperature)
        &&detail::finite(result.criticalDensity)
        &&detail::finite(result.densitySlope)
...
template<int NC,int NR>
PINTLE_HD inline bool cacheMatches(
    const PintleDevicePR::Table<NC,NR>& table,
    ...
    return buildPureReportedPhaseCache(table,expected)==Status::Success
        ...
        &&cache.densitySlope==expected.densitySlope;
}

PINTLE_HD inline Status reportedPhaseCachedTrusted(
    double T,double rho,const PureReportedPhaseCache& cache,int& phase) {
    if(cache.valid!=1||!PintleDevicePR::detail::finite(T)
       ||!PintleDevicePR::detail::finite(rho)||T<=0||rho<=0)
        return Status::InvalidInput;
    if(T>=cache.criticalTemperature){
        phase=int(PintleDevicePR::Phase::Supercritical);
        return Status::Success;
    }
    const double rhoMid=cache.criticalDensity
        +(T-cache.criticalTemperature)*cache.densitySlope;
    phase=rho<rhoMid?int(PintleDevicePR::Phase::Gas)
                    :int(PintleDevicePR::Phase::Liquid);
    return PintleDevicePR::detail::finite(rhoMid)
        ?Status::Success:Status::NonFinite;
}

template<int NC,int NR>
PINTLE_HD inline Status reportedPhaseCachedWithFallback(
...
// invariant cache.
template<int NC,int NR>
PINTLE_HD inline Status evaluateTrhoFromMixingCachedPhase1(
