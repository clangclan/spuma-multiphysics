...
// Included inside PintleDevicePR::detail after NasaRegion is declared.
struct DoubleSingle {
    float hi,lo;
    PINTLE_HD DoubleSingle(double x=0):hi(float(x)),lo(float(x-double(hi))){}
    PINTLE_HD static DoubleSingle parts(float a,float b){DoubleSingle r;r.hi=a;r.lo=b;return r;}
    PINTLE_HD static DoubleSingle renormalize(float a,float b){const float s=a+b;return parts(s,b-(s-a));}
    PINTLE_HD explicit operator double()const{return double(hi)+double(lo);}
    PINTLE_HD friend DoubleSingle operator+(DoubleSingle a,DoubleSingle b){
        const float s=a.hi+b.hi,v=s-a.hi;
        const float e=(a.hi-(s-v))+(b.hi-v)+a.lo+b.lo;
        return renormalize(s,e);
    }
    PINTLE_HD friend DoubleSingle operator-(DoubleSingle a){return parts(-a.hi,-a.lo);}
    PINTLE_HD friend DoubleSingle operator-(DoubleSingle a,DoubleSingle b){return a+(-b);}
    PINTLE_HD friend DoubleSingle operator*(DoubleSingle a,DoubleSingle b){
...
PINTLE_HD inline bool mixedNasa(const NasaRegion& region,double temperature,
