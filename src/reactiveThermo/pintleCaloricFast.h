...
#ifndef PINTLE_CALORIC_FAST_H
#define PINTLE_CALORIC_FAST_H
...
struct PintleCaloricData {
    struct Species {int polynomial=0;double gasConstant=0;std::vector<PintleGasThermoRegion> regions;};
    ...
    explicit PintleCaloricData(Cantera::ThermoPhase& gas) {
        try {
            ...
            for(size_t k=0;k<n;++k) {
                ...
                auto append=[&](double l,double h,const double* coeff,size_t count){PintleGasThermoRegion r{};
...
struct PintleFixedCaloric {
    const PintleCaloricData& d;PintleCostProfileV21& cost;Vector Y,x;
    ...
    PintleFixedCaloric(const PintleCaloricData& data,const Vector& y,const Vector& weights,PintleCostProfileV21& stats)
