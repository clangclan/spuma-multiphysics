...
// Included inside ReactiveDevicePR::detail. Only initial transcendentals use
// FP32; cubic coefficients, topology, refinement and acceptance remain FP64.
REACTIVE_HD inline bool mixedCubicRoots(double center,double delta,double delta2,
    double q,double h,double disc,double bn,double cn,double dn,double b,double* raw,int& count) {
#if defined(__CUDA_ARCH__) && REACTIVE_HEM_FP32_SEEDS
