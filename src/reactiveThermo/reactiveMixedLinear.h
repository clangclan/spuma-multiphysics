...
#ifndef REACTIVE_MIXED_LINEAR_H
#define REACTIVE_MIXED_LINEAR_H
#include <cfloat>
#include <cmath>
#include <cstdint>
#include <climits>
namespace ReactiveMixedLinear {
...
struct Fixed {
    ...
    REACTIVE_HD Fixed():q(0){}
    REACTIVE_HD Fixed(double x):q(INT_MIN){
        if(std::isfinite(x)&&::fabs(x)<31.999999){const int64_t v=::llrint(x*double(scale));
            if(v>INT_MIN&&v<=INT_MAX)q=int32_t(v);}
    }
    REACTIVE_HD static Fixed raw(int64_t x){Fixed v;v.q=x>INT_MIN&&x<=INT_MAX?int32_t(x):INT_MIN;return v;}
    REACTIVE_HD bool valid()const{return q!=INT_MIN;}
    REACTIVE_HD explicit operator double()const{return valid()?double(q)/double(scale):HUGE_VAL;}
    REACTIVE_HD static Fixed quotient(int64_t a,int64_t b){
    ...
    REACTIVE_HD friend Fixed operator-(Fixed a){return a.valid()?raw(-int64_t(a.q)):a;}
    REACTIVE_HD friend Fixed operator+(Fixed a,Fixed b){return a.valid()&&b.valid()?raw(int64_t(a.q)+b.q):raw(INT_MIN);}
    REACTIVE_HD friend Fixed operator-(Fixed a,Fixed b){return a+(-b);}
    REACTIVE_HD friend Fixed operator*(Fixed a,Fixed b){return a.valid()&&b.valid()?quotient(int64_t(a.q)*b.q,scale):raw(INT_MIN);}
    REACTIVE_HD friend Fixed operator/(Fixed a,Fixed b){return a.valid()&&b.valid()?quotient(int64_t(a.q)*scale,b.q):raw(INT_MIN);}
    REACTIVE_HD Fixed& operator-=(Fixed b){return *this=*this-b;}
    REACTIVE_HD Fixed& operator/=(Fixed b){return *this=*this/b;}
    REACTIVE_HD friend bool operator<(Fixed a,Fixed b){return a.valid()&&b.valid()&&a.q<b.q;}
    REACTIVE_HD friend bool operator>(Fixed a,Fixed b){return a.valid()&&b.valid()&&a.q>b.q;}
};
REACTIVE_HD inline float absolute(float x){return ::fabsf(x);}
REACTIVE_HD inline Fixed absolute(Fixed x){return x.q<0?-x:x;}
REACTIVE_HD inline bool valid(float x){return std::isfinite(x);}
REACTIVE_HD inline bool valid(Fixed x){return x.valid();}
REACTIVE_HD inline bool castLost(float x,double source){return source!=0&&::fabsf(x)<FLT_MIN;}
REACTIVE_HD inline bool castLost(Fixed x,double){return !x.valid();}

template<class Scalar>
REACTIVE_HD inline bool solve(const double matrix[4][4],const double* rhs,int n,double* result) {
