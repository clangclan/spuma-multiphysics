...
#include "reactiveDevicePRPurePhaseCache.h"
#include "reactiveDevicePRSelectedMu.h"
#include "reactiveDeviceFlashJacobian.h"
#include "reactiveThermo.h"
#include "reactiveFlashSeeds.h"
...
#endif
#ifndef REACTIVE_HEM_FP32_LINEAR
#define REACTIVE_HEM_FP32_LINEAR 0
#endif
#ifndef REACTIVE_HEM_TP_REUSE_GAS
#define REACTIVE_HEM_TP_REUSE_GAS 1
#endif
#ifndef REACTIVE_HEM_TP_REUSE_LIQUID
#define REACTIVE_HEM_TP_REUSE_LIQUID 1
#endif
#ifdef __CUDACC__
#define REACTIVE_HEM_CALL __host__ __device__ __noinline__
#else
#define REACTIVE_HEM_CALL
#endif
namespace ReactiveDeviceFlash {
using Table=ReactiveDevicePR::Table<>;
...
struct Output {ReactiveThermoState state{};Counters counters{};int success=0,status=0;};
struct Evaluation {
...
REACTIVE_HD inline double lo(double a,double b){return a<b?a:b;}
REACTIVE_HD inline double hi(double a,double b){return a>b?a:b;}
REACTIVE_HD inline double clamp(double a,double l,double h){return lo(hi(a,l),h);}
REACTIVE_HD inline bool finite(double x){return x==x&&x<=DBL_MAX&&x>=-DBL_MAX;}
...
inline bool validModel(const Model& m){
    ...
    for(int i=0;i<=m.nl;++i)
        if(ReactiveDevicePR::detail::validate(m.phase[i])!=ReactiveDevicePR::Status::Success)return false;
    ...
    for(int i=0;i<m.nl;++i){
        ...
        if(m.condensedKind[i]&&!ReactiveSolidThermo::validModel(m.solid[i]))return false;
...
REACTIVE_HD inline double norm(const double* a,int n){double r=0;for(int i=0;i<n;++i)r=hi(r,::fabs(a[i]));return r;}
// Full row/column pivoting, matching Eigen FullPivLU rank criterion for n<=4.
REACTIVE_HD inline bool solve(const double matrix[4][4],const double* rhs,int n,double* x,Counters* counters=nullptr){
#if defined(__CUDA_ARCH__) && (REACTIVE_HEM_FP32_LINEAR || REACTIVE_HEM_INT_LINEAR)
    if(counters){++counters->mixedLinearAttempts;
    using LinearScalar=
#if REACTIVE_HEM_INT_LINEAR
        ReactiveMixedLinear::Fixed;
#else
        float;
#endif
    if(ReactiveMixedLinear::solve<LinearScalar>(matrix,rhs,n,x)){
...
struct Flash {
    ...
    const ReactiveDevicePRPurePhaseCache::PureReportedPhaseCache* liquidCache=nullptr;
    REACTIVE_HD Flash(const Model& model,const Input& input,Counters& counters,
        const ReactiveDevicePRPurePhaseCache::PureReportedPhaseCache* cache=nullptr,
    ...
    REACTIVE_HD double gasPressure(double pbar)const{return pbar-color*pressureJump;}
    REACTIVE_HD double liquidPressure(double pbar)const{return pbar+(1-color)*pressureJump;}
    REACTIVE_HD double density()const{double r=0;for(int k=0;k<m.ns;++k)r+=in.q[k];return r;}
    REACTIVE_HD bool hasSolid()const{return m.nl==2&&m.condensedKind[1]==1;}
    REACTIVE_HD bool bounds(double p,double T)const{return finite(p)&&finite(T)&&p>=m.pmin&&p<=m.pmax&&T>=m.Tmin&&T<=m.Tmax;}
    REACTIVE_HEM_CALL ReactiveDevicePR::Status phaseStatus(int index,double p,double T,const double* Y,ReactiveDevicePR::State& out,bool chemicalPotentials=true){
        using Status=ReactiveDevicePR::Status;
        ...
        if(index>=0&&m.condensedKind[index]){
            ...
            if(!ReactiveSolidThermo::evaluate(s,T,p,v))return Status::InvalidInput;
            out=ReactiveDevicePR::State{};out.T=T;out.p=p;out.W=s.molecularWeight;out.V=s.molarVolume;
...
#if REACTIVE_HEM_TP_REUSE_GAS
        if(index<0&&m.ns==4&&m.nl==2&&chemicalPotentials){
            unsigned muMask=0;
            for(int i=0;i<2;++i)if(in.q[m.condensable[i]]>0)
                muMask|=1u<<m.condensable[i];
            return ReactiveDevicePRSelectedMu::evaluateTPTrustedFixed4(
                m.phase[0],T,p,Y,ReactiveDevicePR::RootChoice::Gas,muMask,out);
        }
        if(index<0&&m.ns==4)
            return ReactiveDevicePR::evaluateTPTrustedFixed4(m.phase[0],T,p,Y,
                ReactiveDevicePR::RootChoice::Gas,out,chemicalPotentials);
#endif
#if REACTIVE_HEM_TP_REUSE_LIQUID
        if(index>=0&&liquidCache)
            return ReactiveDevicePRPurePhaseCache::evaluateTPTrustedFixed1(
                m.phase[index+1],T,p,pure,ReactiveDevicePR::RootChoice::Liquid,
                liquidCache[index],out,chemicalPotentials);
        if(index>=0)
            return ReactiveDevicePR::evaluateTPTrustedFixed1(m.phase[index+1],T,p,pure,
                ReactiveDevicePR::RootChoice::Liquid,out,chemicalPotentials);
#endif
        return ReactiveDevicePR::evaluateTPTrusted(m.phase[index+1],T,p,index<0?Y:pure,
            index<0?ReactiveDevicePR::RootChoice::Gas:ReactiveDevicePR::RootChoice::Liquid,out,chemicalPotentials);
    }
    REACTIVE_HD bool phase(int index,double p,double T,const double* Y,ReactiveDevicePR::State& out,bool chemicalPotentials=true){
        return phaseStatus(index,p,T,Y,out,chemicalPotentials)==ReactiveDevicePR::Status::Success;
    }
    REACTIVE_HD bool unavailable(ReactiveDevicePR::Status s)const{
        return s==ReactiveDevicePR::Status::PhaseUnavailable||s==ReactiveDevicePR::Status::NoPhysicalRoot;
    }
    REACTIVE_HD void accumulate(Evaluation& v,double mass,const ReactiveDevicePR::State& p,double phasePressure){
    ...
    REACTIVE_HEM_CALL bool evaluate(const double* mass,double p,double T,Evaluation& v,bool virtualLiquids=false,const double* vapor=nullptr){
        ...
        if(mg>0){for(int k=0;k<m.ns;++k)gas[k]/=mg;ReactiveDevicePR::State prop{};if(!phase(-1,pg,T,gas,prop,virtualLiquids))return false;
        ...
        for(int i=0;i<m.nl;++i){s.liquidMass[i]=mass[i];
            if(mass[i]>0||(virtualLiquids&&in.q[m.condensable[i]]>0)){
                ReactiveDevicePR::State prop{};ReactiveDevicePR::Status status;
                ...
                if(i==0&&unavailableStatus>=0&&p==unavailableP&&T==unavailableT
                   &&virtualLiquids==unavailableChemical){
                    ++count.phases;status=static_cast<ReactiveDevicePR::Status>(unavailableStatus);
                }else{
                ...
                if(status!=ReactiveDevicePR::Status::Success){
    ...
    REACTIVE_HD double energyScale(const Evaluation& v)const{return hi(1e5*v.state.rho,v.cpDensity*v.state.T);}
    REACTIVE_HEM_CALL bool gasProbe(double T,double& F,double& cv,double& p,double* cp=nullptr){
        const double rho=density();double Y[maxSpecies]{};for(int k=0;k<m.ns;++k)Y[k]=in.q[k]/rho;
        ReactiveDevicePR::State prop{};++count.phases;
        if(ReactiveDevicePR::evaluateTrhoTrusted(m.phase[0],T,rho,Y,prop,false)!=ReactiveDevicePR::Status::Success)return false;
        ...
        ReactiveDevicePR::State gas{};if(!phase(-1,gasP,T,Y,gas,false)||::fabs(gas.rho-rho)>1e-8*rho)return false;
        F=rho*prop.e-in.energy;cv=rho*prop.cv;if(cp)*cp=prop.cp;
        return finite(F)&&finite(cv)&&cv>0;
    }
    REACTIVE_HEM_CALL bool scalarGas(Evaluation& result){
    ...
    REACTIVE_HEM_CALL bool activeFlash(const int* active,int n,double seed,Evaluation& value,double secondSeed=-2,
                                    const ReactiveThermoState* restart=nullptr){
    ...
    REACTIVE_HEM_CALL bool stable(Evaluation& value){
        ...
        if(hasSolid()){
            ...
            ReactiveDevicePR::State liquid{},solidState{};
            ...
            if(st==ReactiveDevicePR::Status::Success){if((solidState.mu[0]-liquid.mu[0])/(8314.46261815324*T)<-m.mutol)return false;}
            else if(!unavailable(st))return false;
        }
        int present[2]{},n=0;double mu[2]{};
        for(int i=0;i<m.nl;++i)if(value.state.liquidMass[i]>0){present[n++]=i;ReactiveDevicePR::State prop{};if(!phase(i,liquidPressure(p),T,nullptr,prop))return false;mu[i]=prop.mu[0]/(8314.46261815324*T);}
        if(!n)return false;
    ...
    REACTIVE_HEM_CALL double tangent(double p,double T,const int* present,int n,const double* mu,double fraction){
        ...
        ReactiveDevicePR::State prop{};const auto status=phaseStatus(-1,gasPressure(p),T,Y,prop,false);
        if(status!=ReactiveDevicePR::Status::Success)return unavailable(status)?HUGE_VAL:NAN;
    ...
    REACTIVE_HEM_CALL bool temperatureSearch(Evaluation& best){
        ...
        for(int mask=1;mask<(1<<m.nl);++mask){
            ...
            for(int t=0;t<ReactiveFlashSeeds::count;++t){
                ReactiveThermoState restart=in.guess;
    ...
    REACTIVE_HEM_CALL bool run(Evaluation& result,bool capillaryBoundary=false){
...
REACTIVE_HD inline Output recover(const Model& model,const Input& input,
    const ReactiveDevicePRPurePhaseCache::PureReportedPhaseCache* cache=nullptr){
...
REACTIVE_HD inline Output recoverCapillary(const Model& model,const Input& input,
...
REACTIVE_HD inline Output initializeCapillaryTP(const Model& model,const Input& input,
