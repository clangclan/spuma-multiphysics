...
#ifndef REACTIVE_DEVICE_WALE_PR_H
#define REACTIVE_DEVICE_WALE_PR_H
#include "reactiveDeviceFlash.h"
#include <cfloat>

namespace ReactiveDeviceWalePr {
enum Status : unsigned {
    Success=0, Scope=1, Input=2, Inventory=3, GasEOS=4, PartialH=5,
    LiquidEOS=6, RecoveredState=7, Energy=8
};
REACTIVE_HD inline double abs(double x){return ::fabs(x);}
REACTIVE_HD inline double maximum(double a,double b){return a>b?a:b;}
REACTIVE_HD inline bool finite(double x){return ReactiveDeviceFlash::finite(x);}

REACTIVE_HD inline Status evaluate(const ReactiveDeviceFlash::Model& model,
    const double* total,const ReactiveThermoState& state,double color,double jump,
    double* effective,double& cp,bool enthalpies)
{
    using namespace ReactiveDeviceFlash;
    ...
    ReactiveDeviceFlash::Input input{};
    ...
    ReactiveDevicePR::State g{},l{};
    if(!phase.phase(-1,pg,state.T,Y,g,false))return GasEOS;
    ReactiveDeviceFlashJacobian::PhasePartials partials{};
    if(!ReactiveDeviceFlashJacobian::phasePartials(model.phase[0],state.T,Y,g,partials))
        return PartialH;
    for(int k=0;k<model.ns;++k){
