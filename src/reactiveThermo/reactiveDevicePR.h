...
#ifndef REACTIVE_DEVICE_PR_H
#define REACTIVE_DEVICE_PR_H
...
#include <cmath>
#include <cfloat>
#ifndef REACTIVE_HEM_FP32_SEEDS
#define REACTIVE_HEM_FP32_SEEDS 0
#endif
#ifndef REACTIVE_HEM_NASA_PRECISION
#define REACTIVE_HEM_NASA_PRECISION 0
#endif
...
REACTIVE_HD inline double abs(double x) { return x < 0 ? -x : x; }
REACTIVE_HD inline double max(double a, double b) { return a > b ? a : b; }
REACTIVE_HD inline double min(double a, double b) { return a < b ? a : b; }
REACTIVE_HD inline bool finite(double x) {
    return x == x && abs(x) <= 1.7976931348623157e308;
}

REACTIVE_HD inline double cubeRoot(double x) {
    return x < 0 ? -::pow(-x, 1.0 / 3.0) : ::pow(x, 1.0 / 3.0);
}

#include "reactiveMixedNasa.h"

REACTIVE_HD inline void nasa(const NasaRegion& region, double T,
                           double& cp_R, double& h_RT, double& s_R)
{
#if defined(__CUDA_ARCH__) && REACTIVE_HEM_NASA_PRECISION == 1
    if(mixedNasa<float>(region,T,cp_R,h_RT,s_R))return;
#elif defined(__CUDA_ARCH__) && REACTIVE_HEM_NASA_PRECISION == 2
...
REACTIVE_HD inline Status validate(const Table<NC, NR>& table)
{
...
template<int NC, int NR>
REACTIVE_HD inline const NasaRegion& region(const Table<NC, NR>& table,
                                           int k, double T)
{
    const auto& sp = table.species[k];
    int r = 0;
...
template<int NC, int NR>
REACTIVE_HD inline Status mixing(const Table<NC, NR>& table, double T,
...
REACTIVE_HD inline double cubicResidual(double V, double bn, double cn, double dn) {
    return ((V + bn)*V + cn)*V + dn;
}

REACTIVE_HD inline void sort3(double* roots, int count) {
...
REACTIVE_HD inline int uniquePhysical(double* roots, int count, double b) {
    sort3(roots, count);
    int n = 0;
...
template<int NC, int NR>
REACTIVE_HD inline Status massToMole(const Table<NC, NR>& table, const double* Y,
                                   double* x, double& W)
{
...
template<int NC, int NR>
REACTIVE_HD inline Status rootsTP(const Table<NC, NR>& table, double T, double p,
...
template<int NC, int NR>
REACTIVE_HD inline Status reportedPhase(const Table<NC, NR>& table, double T,
...
template<int NC, int NR>
REACTIVE_HD inline Status evaluateTrhoTrusted(const Table<NC, NR>& table, double T,
...
template<int NC, int NR>
REACTIVE_HD inline Status evaluateTrho(const Table<NC, NR>& table, double T,
                                     double rho, const double* Y, State& output,
                                     bool chemicalPotentials = true)
{
...
template<int NC, int NR>
REACTIVE_HD inline Status evaluateTPTrusted(const Table<NC, NR>& table, double T, double p,
...
REACTIVE_HD inline Status mixingFixed(const Table<NC, NR>& table, double T,
                               const double* x, double& a, double& b,
                               double& aa, double& da, double& d2a)
{
...
template<int NS, int NC, int NR>
REACTIVE_HD inline Status mixingRootsFixed(const Table<NC, NR>& table, double T,
                                         const double* x, double& a, double& b,
                                         double& aa)
{
    if (NS < 1 || NC < NS || table.ns != NS) return Status::InvalidTable;
...
REACTIVE_HD inline Status rootsFromMixingCached(
...
template<int NS, int NC, int NR>
REACTIVE_HD inline Status reportedPhaseCached(const Table<NC, NR>& table, double T,
                                      const double* x, double W, double rho,
                                      double mixtureA, double mixtureB, int& phase)
{
    const double tc = mixtureA*OmegaB/(mixtureB*OmegaA*GasConstant);
...
template<int NS, int NC, int NR>
REACTIVE_HD inline Status evaluateTrhoFromMixingCached(
...
template<int NS, int NC, int NR>
REACTIVE_HD inline Status evaluateTPTrustedCachedFixed(const Table<NC, NR>& table, double T, double p,
                                          const double* Y, RootChoice choice, State& output,
                                          bool chemicalPotentials = true)
{
    if (NS < 1 || NC < NS || table.ns != NS) return Status::InvalidTable;
...
template<int NC, int NR>
REACTIVE_HD inline Status evaluateTP(const Table<NC, NR>& table, double T, double p,
                                   const double* Y, RootChoice choice, State& output,
                                   bool chemicalPotentials = true)
{
