...
#ifndef REACTIVE_GPU_HEM_CANDIDATES_CUH
#define REACTIVE_GPU_HEM_CANDIDATES_CUH

#ifdef __CUDACC__

#include "../reactiveThermo/reactiveDeviceFlash.h"

namespace ReactiveGpuHemCandidates {

using ReactiveDeviceFlash::Counters;
using ReactiveDeviceFlash::Evaluation;
using ReactiveDeviceFlash::Flash;
...
__device__ inline bool inputValid(const Model& model, const Input& input)
{
    ...
    if (model.ns <= 0 || model.ns > ReactiveDeviceFlash::maxSpecies
        || model.nl < 0 || model.nl > 2
        || !ReactiveDeviceFlash::finite(input.energy)) {
        return false;
    }
    double density = 0;
    for (int k = 0; k < model.ns; ++k) {
        if (!ReactiveDeviceFlash::finite(input.q[k]) || input.q[k] < 0) return false;
        density += input.q[k];
    }
    return density > 0;
}
...
#endif // __CUDACC__
#endif // REACTIVE_GPU_HEM_CANDIDATES_CUH
