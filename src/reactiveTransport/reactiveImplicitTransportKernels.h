...
#ifndef REACTIVE_IMPLICIT_TRANSPORT_KERNELS_H
#define REACTIVE_IMPLICIT_TRANSPORT_KERNELS_H
#include "reactiveTransportKernels.h"
#include "../reactiveInterface/reactiveImplicitFace.h"
#ifdef __CUDACC__
#define REACTIVE_IT_HD __host__ __device__
#else
#define REACTIVE_IT_HD
#endif
...
struct ImplicitVolumeMismatch {
    ...
    REACTIVE_IT_HD double operator()(size_t c)const {
        return fabs(cells[c].liquidVolume*view.inverseVolume[c]-view.capillaryColor[c])>tolerance?1:0;
    }
};
struct ImplicitTarget {
    double* output;
    REACTIVE_IT_HD void operator()(size_t logical,View v)const {
        output[logical]=v.capillaryColor[v.cartesian.logicalToCell[logical]];
    }
};
REACTIVE_IT_HD inline ReactiveImplicitSurface::View implicitField(View v,const double* coefficient) {
    ReactiveImplicitSurface::View field{};field.coefficients=coefficient;
...
struct ImplicitCells {
    ...
    REACTIVE_IT_HD void operator()(size_t logical,View v)const {
        uint64_t coordinates[3];ReactiveCartesianMesh::logicalCoordinates(v.cartesian,logical,coordinates);
        const int64_t cell[3]={int64_t(coordinates[0]),int64_t(coordinates[1]),int64_t(coordinates[2])};
        ReactiveImplicitVolume::Integrals integral{};
        if(!ReactiveImplicitVolume::integrate(implicitField(v,coefficient),cell,{tolerance,12},integral)) {
...
struct ImplicitFaces {
    ...
    ReactiveGeometricFaceV1* output;
    REACTIVE_IT_HD void operator()(size_t fi,View v)const {
        ...
        uint64_t coordinates[3];ReactiveCartesianMesh::logicalCoordinates(v.cartesian,
            uint64_t(v.cartesian.cellToLogical[f.owner]),coordinates);
        const int64_t cell[3]={int64_t(coordinates[0]),int64_t(coordinates[1]),int64_t(coordinates[2])};
        ReactiveImplicitFace::Integrals integral{};
        const auto field=implicitField(v,coefficient);
        if(!ReactiveImplicitFace::integrate(field,cell,axis,side,{tolerance,12},integral)) {
            v.failCapillary();return;
        }
        ...
        if(f.neighbour>=0&&((side&&coordinates[axis]+1==v.cartesian.dims[axis])
            ...
            uint64_t nc[3];ReactiveCartesianMesh::logicalCoordinates(v.cartesian,
                uint64_t(v.cartesian.cellToLogical[f.neighbour]),nc);
            const int64_t neighbour[3]={int64_t(nc[0]),int64_t(nc[1]),int64_t(nc[2])};
            ReactiveImplicitFace::Integrals peer{};
            if(!ReactiveImplicitFace::integrate(field,neighbour,axis,1-side,{tolerance,12},peer)
            ...
            if(integral.curveLength>0||peer.curveLength>0
               ||(integral.liquidArea>0&&integral.liquidArea<f.area)) {
                ...
                for(int i=0;i<4;++i)for(int j=0;j<4;++j){
                    ...
                    ReactiveImplicitSurface::Sample a{},p{};
                    if(!ReactiveImplicitSurface::evaluateCell(field,cell,local,a)){v.failCapillary();return;}
                    local[axis]=1-side;
                    if(!ReactiveImplicitSurface::evaluateCell(field,neighbour,local,p)){v.failCapillary();return;}
        ...
        ReactiveGeometricFaceV1 out{};out.liquidArea=integral.liquidArea;
...
struct ImplicitCommit {
    ...
    REACTIVE_IT_HD void operator()(size_t c,View v)const {
