...
#include "../reactiveThermo/pintleClosureAcceleration.h"
#include <chrono>
#include <cstdio>
#include <memory>
#ifdef __CUDACC__
...
__device__ bool closureValue(const PintleClosureScalarInputV1& in,double T,
...
__global__ void closureScalarKernel(const PintleClosureScalarInputV1* inputs,size_t count,PintleClosureScalarOutputV1* outputs)
{
    const size_t i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=count)return;
    const auto in=inputs[i];PintleClosureScalarOutputV1 out{};
...
struct ClosureDevice {
    size_t capacity=0;PintleClosureScalarInputV1* input=nullptr;PintleClosureScalarOutputV1* output=nullptr;
...
extern "C" void* pintle_closure_scalar_create_v1(size_t n,char* error,size_t size){
...
extern "C" void pintle_closure_scalar_destroy_v1(void* p){delete static_cast<ClosureDevice*>(p);}
extern "C" int pintle_closure_scalar_run_v1(void* p,const PintleClosureScalarInputV1* in,size_t n,
    PintleClosureScalarOutputV1* out,PintleClosureDeviceProfileV1* profile,char* error,size_t size){
...
#else
extern "C" void* pintle_closure_scalar_create_v1(size_t,char* error,size_t size){
    if(error&&size)std::snprintf(error,size,"Transport library was built without CUDA scalar closure");
    return nullptr;}
extern "C" void pintle_closure_scalar_destroy_v1(void*){}
extern "C" int pintle_closure_scalar_run_v1(void*,const PintleClosureScalarInputV1*,size_t,PintleClosureScalarOutputV1*,PintleClosureDeviceProfileV1*,char*,size_t){return 1;}
#endif
