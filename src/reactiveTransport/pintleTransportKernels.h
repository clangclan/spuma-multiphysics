...
struct View {
    ...
    PINTLE_HD double faceNut(const PintleTransportFace& f,bool right) const {
        if(!wale)return 0;
    ...
    PINTLE_HD double sgsSpeciesFlux(size_t fi,size_t k) const {
    ...
    PINTLE_HD double liquidSgsFlux(size_t fi) const {
...
struct InterfaceGradient {
    PINTLE_HD void operator()(size_t c,View v) const {
        if(!PintleUnstructuredInterface::gradientCell(c,v.interface))v.failCapillary();
    }
};
struct InterfaceCurvature {
    PINTLE_HD void operator()(size_t c,View v) const {
        if(!PintleUnstructuredInterface::curvatureCell(c,v.interface))v.failCapillary();
...
struct WaleViscosities {
    PINTLE_HD void operator()(size_t c,View v) const {
        ...
        const bool ok=PintleWale::filterWidthFromCellVolume(1/v.inverseVolume[c],delta)
            &&PintleWale::eddyViscosity(v.gradient+c*9,delta,v.waleCw,nut);
...
struct Faces {
    PINTLE_HD void operator()(size_t fi,View v) const {
        ...
        PintleBalancedCapillary::Flux capFlux{};
        if(v.capillary) {
            PintleBalancedCapillary::State csL{},csR{};
