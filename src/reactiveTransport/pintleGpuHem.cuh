...
#include "../reactiveThermo/pintleGpuHem.h"
#ifdef __CUDACC__
#include "../reactiveThermo/pintleDeviceFlash.h"
#ifndef PINTLE_HEM_CANDIDATE_PARALLEL
#define PINTLE_HEM_CANDIDATE_PARALLEL 0
#endif
#ifndef PINTLE_HEM_PHASE_BUCKETS
#define PINTLE_HEM_PHASE_BUCKETS 1
#endif
static_assert(PINTLE_HEM_PHASE_BUCKETS==0||PINTLE_HEM_PHASE_BUCKETS==1,
    "Phase bucket selection must be 0 or 1");
static_assert(PINTLE_HEM_CANDIDATE_PARALLEL==0,
    "The compact bridge preserves the accepted one-thread-per-cell HEM kernel only");
namespace {
using HemModel=PintleDeviceFlash::Model;
using HemInput=PintleDeviceFlash::Input;
using HemOutput=PintleDeviceFlash::Output;
using HemCounters=PintleDeviceFlash::Counters;
using HemPhaseCache=PintleDevicePRPurePhaseCache::PureReportedPhaseCache;
static_assert(2*sizeof(HemPhaseCache)==PINTLE_GPU_HEM_PHASE_CACHE_BYTES_V1,
...
__global__ void initializeHemPhaseCache(const HemModel* model,HemPhaseCache* cache){
    ...
    for(int i=0;i<2;++i){cache[i]=HemPhaseCache{};
        if(i<model->nl)PintleDevicePRPurePhaseCache::buildPureReportedPhaseCache(
            model->phase[i+1],cache[i]);}
}
#ifndef PINTLE_HEM_BLOCK_THREADS
#define PINTLE_HEM_BLOCK_THREADS 64
#endif
#ifndef PINTLE_HEM_MIN_BLOCKS
#define PINTLE_HEM_MIN_BLOCKS 1
#endif
static_assert(PINTLE_HEM_BLOCK_THREADS>=32&&PINTLE_HEM_BLOCK_THREADS<=256
    &&PINTLE_HEM_BLOCK_THREADS%32==0,"Invalid HEM block size");
...
void hemKernel(const HemModel* model,const double* q,const double* energy,
    ...
    if(c<count){const size_t source=order?order[c]:c;HemInput in{};
        ...
        const HemOutput out=capillary
            ?PintleDeviceFlash::recoverCapillary(*model,in,capillary[source].color,
                capillary[source].pressureJump,capillary[source].equilibrium!=0,cache)
            :PintleDeviceFlash::recover(*model,in,cache);
...
struct HemDevice {
    ...
    PintleGpuHemCapillaryInputV2* capillary=nullptr;
...
unsigned hemBucket(const double* q,int ns,const PintleThermoState& guess){
    if(ns!=4)return 0;
...
extern "C" void* pintle_gpu_hem_create_v1(const void* raw,size_t bytes,size_t capacity,char* error,size_t size){
    try{if(!raw||bytes!=sizeof(HemModel)||!capacity||capacity>1048576)throw std::runtime_error("Invalid CUDA HEM model/capacity");
        ...
        if(!PintleDeviceFlash::validModel(model))throw std::runtime_error("Invalid CUDA HEM model tables or domain");
        ...
        closureCuda(cudaMalloc(reinterpret_cast<void**>(&d->guess),capacity*sizeof(PintleThermoState)));
        ...
        closureCuda(cudaMalloc(reinterpret_cast<void**>(&d->phaseCache),
            PINTLE_GPU_HEM_PHASE_CACHE_BYTES_V1));
...
static int hemRun(void* raw,const double* q,const double* energy,
    ...
    try{if(!raw||!q||!energy||!states||!success||!profile||profile->abiVersion!=1||profile->structBytes!=sizeof(*profile))throw std::runtime_error("Invalid CUDA HEM batch arguments");
        ...
        closureCuda(cudaMemcpyAsync(d.guess,states,count*sizeof(PintleThermoState),cudaMemcpyHostToDevice,d.stream));
        if(capillary)closureCuda(cudaMemcpyAsync(d.capillary,capillary,
            count*sizeof(PintleGpuHemCapillaryInputV2),cudaMemcpyHostToDevice,d.stream));
#if PINTLE_HEM_PHASE_BUCKETS
...
extern "C" int pintle_gpu_hem_run_v1(void* raw,const double* q,const double* energy,size_t count,
    PintleThermoState* states,int* success,PintleGpuHemProfileV1* profile,char* error,size_t size){
    return hemRun(raw,q,energy,nullptr,count,states,success,profile,error,size);
}
extern "C" int pintle_gpu_hem_run_v2(void* raw,const double* q,const double* energy,
