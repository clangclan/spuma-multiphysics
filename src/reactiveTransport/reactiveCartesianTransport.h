...
#ifndef REACTIVE_CARTESIAN_TRANSPORT_H
#define REACTIVE_CARTESIAN_TRANSPORT_H
...
typedef struct ReactiveCartesianMeshV1 {
    ...
    const ReactiveTransportFace* faces; // length faces, same ordering as create()
} ReactiveCartesianMeshV1;

typedef struct ReactiveCartesianInfoV1 {
...
int reactive_transport_install_cartesian_v1(void* transport,const ReactiveCartesianMeshV1* mesh);
int reactive_transport_cartesian_info_v1(void* transport,ReactiveCartesianInfoV1* info);

typedef struct ReactiveImplicitOptionsV1 {
    uint32_t abiVersion,structBytes;
    double quadratureTolerance,volumeTolerance,smoothness,linearTolerance;
    uint32_t nonlinearIterations,linearIterations;
} ReactiveImplicitOptionsV1;
...
int reactive_transport_set_implicit_v1(void* transport,const ReactiveImplicitOptionsV1* options);
...
int reactive_transport_implicit_coefficients_v1(void* transport,double* output,
