...
#ifndef REACTIVE_IMPLICIT_FACE_H
#define REACTIVE_IMPLICIT_FACE_H
#include "reactiveImplicitVolume.h"
#ifdef __CUDACC__
#define REACTIVE_IF_HD __host__ __device__
#else
#define REACTIVE_IF_HD
#endif
namespace ReactiveImplicitFace {
...
using Options=ReactiveImplicitVolume::Options;
using Patch=ReactiveImplicitVolume::Patch;
...
REACTIVE_IF_HD inline void faceBernstein(const View& field,const int64_t cell[3],
    int axis,int side,double out[64]) {
    ReactiveImplicitVolume::basisToBernstein(field,cell,out);
    const int a=(axis+1)%3,b=(axis+2)%3;
    for(int j=0;j<4;++j)for(int i=0;i<4;++i) {
        int p[3]{};p[a]=i;p[b]=j;p[axis]=3*side;
        const double value=out[ReactiveImplicitVolume::idx(p[0],p[1],p[2])];
        for(int k=0;k<4;++k){p[axis]=k;out[ReactiveImplicitVolume::idx(p[0],p[1],p[2])]=value;}
    }
}
REACTIVE_IF_HD inline bool rule(const View& field,const int64_t cell[3],int axis,
    int side,const Patch& patch,int order,bool transpose,double out[6],unsigned& evaluations) {
    using namespace ReactiveImplicitVolume;
    ...
    for(int segment=0;segment+1<count;++segment) {
        ...
        for(int i=0;i<order;++i) {
            ...
            for(int r=0;r<=roots.count;++r) {
                ...
                if(bounded>prior&&ReactiveImplicitSurface::value(poly,.5*(prior+bounded))<0)
            ...
            for(int r=0;r<roots.count;++r)if(roots.value[r]>=v0&&roots.value[r]<=v1) {
                local[ray]=roots.value[r];ReactiveImplicitSurface::Sample s{};
                if(!ReactiveImplicitSurface::evaluateCell(field,cell,local,s))return false;
                ...
                if(!ReactiveImplicitVolume::finite(magnitude)||magnitude<=0||denominator<=1e-12*magnitude)return false;
    ...
    for(int q=0;q<6;++q)if(!ReactiveImplicitVolume::finite(out[q]))return false;
    return true;
}
...
REACTIVE_IF_HD inline bool integrate(const View& field,const int64_t cell[3],
    int axis,int side,const Options& options,Integrals& output) {
    using namespace ReactiveImplicitVolume;
    if(!ReactiveImplicitSurface::validCell(field,cell)||axis<0||axis>2||side<0||side>1
       ||!ReactiveImplicitVolume::finite(options.tolerance)||options.tolerance<=0||options.tolerance>=.1
    ...
    for(int q=0;q<64;++q)if(!ReactiveImplicitVolume::finite(base[q]))return false;
    ...
    if(!ReactiveImplicitVolume::finite(result.liquidArea)||result.liquidArea < -roundoff||result.liquidArea>area+roundoff)return false;
    if(result.liquidArea<0)result.liquidArea=0;
    if(result.liquidArea>area)result.liquidArea=area;
    output=result;return true;
}
} // namespace ReactiveImplicitFace
#undef REACTIVE_IF_HD
#endif
