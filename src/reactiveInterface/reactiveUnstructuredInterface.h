...
#ifndef REACTIVE_UNSTRUCTURED_INTERFACE_H
#define REACTIVE_UNSTRUCTURED_INTERFACE_H

#include "reactiveBalancedCapillary.h"
#include "../reactiveTransport/reactiveTransport.h"
#include <cstddef>
#include <cstdint>

#ifdef __CUDACC__
#define REACTIVE_UI_HD __host__ __device__
#else
#define REACTIVE_UI_HD
#endif

namespace ReactiveUnstructuredInterface {

struct View {
    std::size_t cells;
    const ReactiveTransportFace* faces;
...
REACTIVE_UI_HD inline double otherColor(const View& v,const ReactiveTransportFace& f) {
...
REACTIVE_UI_HD inline double faceColor(const View& v,const ReactiveTransportFace& f) {
...
REACTIVE_UI_HD inline bool gradientCell(std::size_t c,const View& v) {
    if(c>=v.cells||!v.faces||!v.row||!v.incidence||!v.inverseVolume
       ||!v.color||!v.gradient||!v.normal||!v.areaDensity
       ||!ReactiveBalancedCapillary::finite(v.color[c])
...
REACTIVE_UI_HD inline void faceNormal(const View& v,const ReactiveTransportFace& f,double out[3]) {
...
REACTIVE_UI_HD inline bool curvatureCell(std::size_t c,const View& v) {
    ...
    if(!ReactiveBalancedCapillary::finite(candidate)) return false;
    v.curvature[c]=candidate;
    return true;
}

REACTIVE_UI_HD inline bool faceGeometry(std::size_t fi,const View& v,
                                      ReactiveBalancedCapillary::Face& out) {
    const auto& f=v.faces[fi];
    const std::size_t l=std::size_t(f.owner),r=f.neighbour>=0?std::size_t(f.neighbour):l;
    if(!v.areaDensity||!v.curvature||!v.normal
       ||!ReactiveBalancedCapillary::finite(v.sigma)||v.sigma<0) return false;
    ReactiveBalancedCapillary::Face candidate{};
    ...
    if(!ReactiveBalancedCapillary::finite(candidate.curvature)
       ||!ReactiveBalancedCapillary::finite(candidate.colorFace)
       ||candidate.colorFace<0||candidate.colorFace>1
       ||!ReactiveBalancedCapillary::finite(scale)) return false;
    out=candidate;return true;
}

} // namespace ReactiveUnstructuredInterface

#undef REACTIVE_UI_HD
#endif
