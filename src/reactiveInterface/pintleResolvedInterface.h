...
#ifndef PINTLE_RESOLVED_INTERFACE_H
#define PINTLE_RESOLVED_INTERFACE_H
...
#ifdef __CUDACC__
#define PINTLE_INTERFACE_HD __host__ __device__
#else
#define PINTLE_INTERFACE_HD
#endif

namespace PintleInterface {
...
PINTLE_INTERFACE_HD inline double absolute(double x) { return x < 0 ? -x : x; }
PINTLE_INTERFACE_HD inline bool finite(double x) { return x == x && x <= DBL_MAX && x >= -DBL_MAX; }

PINTLE_INTERFACE_HD inline Model defaultModel(double sigma)
{
    return Model{resolvedImmiscibleConstantSigmaV1, sigma, 1e-12, 1.0};
}

PINTLE_INTERFACE_HD inline Status validate(const Model& model)
{
...
PINTLE_INTERFACE_HD inline Status geometry(
...
PINTLE_INTERFACE_HD inline Status capillaryStress(
...
PINTLE_INTERFACE_HD inline Status capillaryFlux(
...
PINTLE_INTERFACE_HD inline Status surfaceEnergyDensity(
...
PINTLE_INTERFACE_HD inline Status capillaryTimeStep(
...
PINTLE_INTERFACE_HD inline Status laplacePressureJump(
...
PINTLE_INTERFACE_HD inline Status phasePressures(
