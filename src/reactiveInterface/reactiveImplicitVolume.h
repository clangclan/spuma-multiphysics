...
#ifndef REACTIVE_IMPLICIT_VOLUME_H
#define REACTIVE_IMPLICIT_VOLUME_H
#include "reactiveImplicitSurface.h"
#include <cfloat>
#include <cmath>
#ifdef __CUDACC__
#define REACTIVE_IV_HD __host__ __device__
#else
#define REACTIVE_IV_HD
#endif
namespace ReactiveImplicitVolume {
...
REACTIVE_IV_HD inline bool finite(double a){return a==a&&fabs(a)<=DBL_MAX;}
REACTIVE_IV_HD inline int idx(int i,int j,int k){return i+4*(j+4*k);}
REACTIVE_IV_HD inline void basisToBernstein(const View& f,const int64_t c[3],double out[64]) {
    double a[64],b[64];
...
REACTIVE_IV_HD inline void split(const double in[4],double t,double left[4],double right[4]) {
...
REACTIVE_IV_HD inline void restrictAxis(double data[64],int axis,double lo,double hi) {
    if(lo==0&&hi==1)return;
...
REACTIVE_IV_HD inline void gauss(int order,int i,double& x,double& w) {
...
REACTIVE_IV_HD inline bool signDefinite(const double values[64],int axis) {
...
REACTIVE_IV_HD inline bool boundaryChart(const double values[64],int ray,int sweep) {
...
REACTIVE_IV_HD inline bool appendRoots(const View& field,const int64_t cell[3],int axis,
    const double local[3],double lo,double hi,double* breaks,int& count,int capacity) {
    ReactiveImplicitSurface::Roots roots{};ReactiveImplicitSurface::Cubic polynomial{};
    if(!ReactiveImplicitSurface::linePolynomial(field,cell,axis,local,polynomial))return false;
    ...
    if(!ReactiveImplicitSurface::lineRoots(polynomial,roots))return false;
...
REACTIVE_IV_HD inline void sortBreaks(double* points,int count) {
...
REACTIVE_IV_HD inline bool innerRule(const View& field,const int64_t cell[3],int ray,
    ...
    for(int segment=0;segment+1<count;++segment) {
        ...
        for(int j=0;j<order;++j) {
            ...
            ReactiveImplicitSurface::Cubic poly{};ReactiveImplicitSurface::Roots roots{};
            if(!ReactiveImplicitSurface::linePolynomial(field,cell,ray,local,poly)
               ||!ReactiveImplicitSurface::lineRoots(poly,roots))return false;
            ++evaluations;double prior=0,fraction=0;
            for(int r=0;r<=roots.count;++r) {
                const double next=r<roots.count?roots.value[r]:1;
                if(ReactiveImplicitSurface::value(poly,.5*(prior+next))<0)fraction+=next-prior;
                prior=next;
            }
            out[0]+=weight*fraction;
            for(int r=0;r<roots.count;++r) {
                local[ray]=roots.value[r];ReactiveImplicitSurface::Sample sample{};
                if(!ReactiveImplicitSurface::evaluateCell(field,cell,local,sample))return false;
                ...
                if(jacobian) {
                    ...
                    for(int d=0;d<3;++d)ReactiveImplicitSurface::basis(local[d],basis[d],first,second);
...
REACTIVE_IV_HD inline bool rule(const View& field,const int64_t cell[3],int ray,
...
REACTIVE_IV_HD inline bool integrate(const View& field,const int64_t cell[3],
    ...
    double middle[3]={.5,.5,.5};ReactiveImplicitSurface::Sample sample{};
    if(!ReactiveImplicitSurface::evaluateCell(field,cell,middle,sample))return false;
