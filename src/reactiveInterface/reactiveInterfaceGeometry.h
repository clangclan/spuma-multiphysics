...
#ifndef REACTIVE_INTERFACE_GEOMETRY_H
#define REACTIVE_INTERFACE_GEOMETRY_H
...
#define REACTIVE_IG_HD __host__ __device__
#else
#define REACTIVE_IG_HD
#endif

namespace ReactiveInterfaceGeometry {
...
REACTIVE_IG_HD inline bool finite(double x);
...
REACTIVE_IG_HD inline Status capillaryFaceFlux(const double stress[6],const double unitNormal[3],
...
REACTIVE_IG_HD inline std::size_t cells(const StructuredGrid& g) {
    return std::size_t(g.nx)*std::size_t(g.ny)*std::size_t(g.nz);
}
REACTIVE_IG_HD inline std::size_t cell(const StructuredGrid& g,int i,int j,int k) {
    return std::size_t(i)+std::size_t(g.nx)*(std::size_t(j)+std::size_t(g.ny)*std::size_t(k));
}
REACTIVE_IG_HD inline bool finite(double x) { return x==x && x<=DBL_MAX && x>=-DBL_MAX; }
REACTIVE_IG_HD inline double minimum(double a,double b) { return a<b?a:b; }
...
REACTIVE_IG_HD inline double sample(const double* q,const StructuredGrid& g,const Boundaries& b,int i,int j,int k) {
...
REACTIVE_IG_HD inline void geometryCell(const double* color,const StructuredGrid& g,const Boundaries& b,
...
REACTIVE_IG_HD inline double sampleComponent(const double* q,const StructuredGrid& g,const Boundaries& b,int i,int j,int k) {
...
REACTIVE_IG_HD inline void capillaryCell(const StructuredGrid& g,const Boundaries& b,double sigma,
...
REACTIVE_IG_HD inline void divergenceCell(const StructuredGrid&g,const Boundaries&b,int i,int j,int k,CapillaryWorkspaceView c){
...
REACTIVE_IG_HD inline std::size_t xface(const StructuredGrid&g,int i,int j,int k){return std::size_t(i)+std::size_t(g.nx+1)*(j+std::size_t(g.ny)*k);}
REACTIVE_IG_HD inline std::size_t yface(const StructuredGrid&g,int i,int j,int k){return std::size_t(i)+std::size_t(g.nx)*(j+std::size_t(g.ny+1)*k);}
REACTIVE_IG_HD inline std::size_t zface(const StructuredGrid&g,int i,int j,int k){return std::size_t(i)+std::size_t(g.nx)*(j+std::size_t(g.ny)*k);}
