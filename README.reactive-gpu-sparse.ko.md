...
`pintleReactiveFoam`에 선택 가능한 CUDA 수송 연산과 CPU 희소 화학 선형계를 추가했다. **기본값은 기존 CPU 수송·밀집 화학이다.** CUDA 소스 컴파일과 독립 연산자/화학 수치 시험은 수행했으나, 이 환경에는 실행 가능한 GPU와 SPUMA가 없어 실제 GPU 실행, 전체 솔버 빌드, 성능 향상은 검증하지 못했다. 병합 전 원래 환경에서 확인해야 하는 개발 단계다.
